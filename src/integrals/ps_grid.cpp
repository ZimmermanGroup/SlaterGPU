...
void reorient_grid(int tid, const double z0, int gs, double* grid, double* grid2, double* rot)
{
 ...
 #pragma acc parallel loop collapse(2) present(grid[0:gs6], rot[0:9], grid2[0:gs6]) async(tid)
  for (int i=0;i<gs;i++)
  {
    for(int j=0;j<3;j++)
    {
...
void initialize_ps_coords_2c_phi(int tid, double a, double cf, int nmu, int nnu, int nphi, double phi_zero, double phi_add, double* grid, double* gridm, double* wt, int prl)
{
 ...
 //c1==3   -> about 100 Bohr
  ...
  #pragma acc enter data copyin(phia[0:nphi],phiad[0:nphi]) async(tid)
 ...
 #pragma acc parallel loop present(grid[0:gs6],gridm[0:gs6],wt[0:gs],phia[0:nphi],phiad[0:nphi]) async(tid)
  ...
  #pragma acc exit data delete(phia[0:nphi],phiad[0:nphi]) async(tid)
...
void shift_boundary_munu(int tid, double mu, double nu, int ix, int jx, int ib, int ia, int jb, int ja, int kb, int ka, int gs, double* gridm)
{
 ...
 //recenters mu/nu so 3rd atom is at boundary of cells, leave phi alone
  #pragma acc serial present(gridm[0:gs6]) async(tid)
  for (int k=0;k<2;k++)
  {
...
void initialize_ps_coords_3c(int tid, double cf, int nmu, int nnu, int nphi, double phi_zero, double* coordn, double* grid, double* gridm, double* wt, double* rot, int prl)
{
 //this code assumes first two atoms aligned along z axis, after the rotation
  ...
  #pragma acc update self(gridm[0:gs6]) async(tid)
  #pragma acc wait

  //double tmp[6];
  //#pragma acc enter data create(tmp[0:6]) async(tid)

 //#pragma acc serial present(tmp[0:6],gridm[0:gs6]) async(tid)
  {
    int ix = 6*nnu*nphi;
    int jx = 6*nphi;
   ...
   //check if munu are near the boundaries already
    ...
    //move shifted grid points to end
    {
      ...
      #pragma acc serial present(gridm[0:gs6],tmp[0:6]) async(tid)
      for (int k=0;k<2;k++)
      {
        int ws = gs-8+k*4;
        int kn = kb; if (k==1) kn = ka;
 ...
 //order munuphi so third atom is "first"
  {
    ...
    int i0 = gs6-48;
   #pragma acc parallel loop independent present(gridm[0:gs6]) async(tid)
    for (int j=0;j<8;j++)
    {
      int i1 = i0+6*j;
...
void refine_3c_grid(int tid, int nsplit, double ztm, double a, double A3, double B3, double C3, int gs, int nsg, double* gridm, double* rot, int prl)
{
 ...
 //put third center in PS plane
  ...
  #pragma acc enter data create(gride[0:48]) async(tid)

  #pragma acc parallel loop collapse(2) present(gridm[0:gs6],gride[0:48]) async(tid)
  for (int j=0;j<8;j++)
  for (int k=0;k<6;k++)
 ...
 #pragma acc parallel loop independent present(gridm[0:gs6],gride[0:48]) async(tid)
  for (int j=0;j<8;j++)
  {
 ...
 #pragma acc parallel loop present(gridm[0:gs6])
  ...
  #pragma acc exit data delete(gride[0:48]) async(tid)
  delete [] gride;
...
void generate_ps_quad_grid_3c_refine(int tid, double cfn, int wb, int nb, double ztm1, double ztm2, int nsplit, int natoms, double* coordn, int quad_order, int quad_r_order, int nmu, int nnu, int nphi, double* grid, double* wt)
{
  ...
  #pragma acc parallel loop present(rot[0:9]) async(tid)
  for (int j=0;j<9;j++)
    rot[j] = 0.;
  #pragma acc parallel loop present(rot[0:9]) async(tid)
  for (int j=0;j<3;j++)
...

  {
   ...
   //if splitting, create rest of grid
    if (nsg>0)
    {
      ...
      #pragma acc enter data copyin(Qxh[0:qoh2],Qyh[0:qoh2],Qzh[0:qoh2]) async(tid)
     ...
     //nsg pts surrounding third nucleus w/refined grid
      ...
      #pragma acc exit data delete(Qxh[0:qoh2],Qyh[0:qoh2],Qzh[0:qoh2]) async(tid)
    }

    reorient_grid(tid,z0,gsq,gridq,grid,rot);
  }

  #pragma acc exit data delete(Qx[0:qo2],Qy[0:qo2],Qz[0:qo2]) async(tid)
  #pragma acc exit data delete(rot[0:9]) async(tid)
  #pragma acc exit data delete(gridq[0:gsq6],gridm[0:gs6]) async(tid)
...
void generate_ps_quad_grid(int tid, double cfn, int wb, int nb, double Z1, int natoms, double* coordn, int quad_order, int quad_r_order, int nmu, int nnu, int nphi, double* grid, double* wt)
{
  ...
  if (natoms==1)
  {
   ...
   //zero weight on unused data pts. also need r1 to be nonzero
    #pragma acc parallel loop present(wt[0:gsq]) async(tid)
    for (int j=0;j<gsq;j++)
      wt[j] = 0.;
    #pragma acc parallel loop present(grid[0:gsq6]) async(tid)
