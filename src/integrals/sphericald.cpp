...
{
  return eval_sh_3rd(-1,gs,grid,val,n1,l1,m1);
}
...
void eval_ss(int tid, int gs, float* gridf, float* valf, int n1, int l1, int m1, double zeta1, double Rc)
{
  int gs6 = 6*gs;
  double* val = new double[gs];
  double* grid = new double[gs6];
 ...
 #pragma acc parallel loop present(val[0:gs],valf[0:gs])
  ...
  eval_ssd(tid,gs,grid,val,n1,l1,m1,zeta1,Rc);
...
#if SS_V3 || SS_V4
//new version
void eval_ssd(int tid, int gs, double* grid, double* val, int n1, int l1, int m1, const double zeta1, double Rc)
{
 #if 0
  if (n1-l1>1)
  {
   //regular Slater ftn
    eval_shd(tid,gs,grid,val,n1,l1,m1,zeta1);
    return;
  }
 #endif
 ...
 #if SS_V5
  double c2 = c*c;
  double f1 = 5.*a4 - a2*c2 - 4.*c4;
  double f2 = (10.*a2-c2);
  ztos = a2/6.* (2. + f1/(a2*f2));
  zt3 = a*f1/(6.*f2);
 #endif
  double zt5 = 0.;
  double zt6 = 0.;
...
void eval_ss_ked(int tid, int gs, double* grid, double* val, int n1, int l1, int m1, const double zeta1, double Rc)
{
  int gs6 = 6*gs;
  double c = 1./Rc;
  double a = zeta1;
  ...
  if (n1-l1>1)
  {
   //regular Slater ftn
    eval_shd(tid,gs,grid,val,n1,l1,m1,zeta1);
    eval_ked(tid,gs,grid,val,n1,l1,zeta1);
    return;
  }
 #endif
...
void eval_ss_pd(int gs, double* grid, double* val, double* tmp, int n1, int l1, int m1, const double zeta1, double Rc)
{
  ...
  {
   //regular Slater ftn
    eval_pd(-1,gs,grid,val,n1,l1,m1,zeta1);
    return;
  }
 #endif
  ...
  for (int n=0;n<ns;n++)
  {
   ...
   #if SS_V4
    //if (n==1) ztn = zt1;
    //if (n==2) ztn = (-a2 + fa4c4)/3.;
    //if (n==3) ztn = (-2.*a2 + fa4c4)*a/3.;
    ...
    double f2 = (10.*a2-c2);
    if (n==1) ztn = zt1;
    if (n==2) ztn = a2/6.* (2. + f1/(a2*f2));
    if (n==3) ztn = a*f1/(6.*f2);
   #endif
    ...
    if (ztn!=0.)
    {
      ...
      eval_pd(-1,gs,grid,tmp,n2,l1,m1,zeta1);
