...
void compute_rho(int natoms, int* atno, float* coords, vector<vector<double> > &basis, double* Pao, int gs, float* grid, double* rho, double* drho, int prl)
{
  ...
  float* grid1a = new float[gsa6];
  float* grid2a = new float[gsa6];

  float* delta = new float[gsa3];

  //#pragma acc exit data delete(grid1a[0:gsa6])
  #pragma acc enter data copyin(grid1a[0:gsa6],grid2a[0:gsa6],delta[0:gsa3])
  #pragma acc enter data create(val1[0:iN][0:gsa],val2[0:iN][0:gsa],val1p[0:iN][0:gsa3],val2p[0:iN][0:gsa3])

  #pragma acc parallel loop present(delta[0:gsa3])
  ...
  for (int m=0;m<natoms;m++)
  {
   //working on this block of the matrix
    ...
    copy_grid(gsa,grid1a,grid);
    recenter_grid_zero(gsa,grid1a,-A1,-B1,-C1);
    ...
    for (int i1=s1;i1<s2;i1++)
    {
      ...
      eval_sh(ii1,gsa,grid1a,val1[ii1],n1,l1,m1,zeta1);
      eval_p(gsa,grid1a,val1p[ii1],n1,l1,m1,zeta1);
    }
   ...
   //single-atom Pao elements added to grid
    for (int i1=s1;i1<s2;i1++)
    {
      ...
      for (int i2=s1;i2<s2;i2++)
      {
       ...
       #pragma acc parallel loop present(delta[0:gsa3],valn[0:gsa],valpn[0:gsa3],valm[0:gsa],valpm[0:gsa3])
        for (int j=0;j<gsa;j++)
        {
          delta[3*j+0] += d1*(valn[j]*valpm[3*j+0]+valpn[3*j+0]*valm[j]);
          delta[3*j+1] += d1*(valn[j]*valpm[3*j+1]+valpn[3*j+1]*valm[j]);
          delta[3*j+2] += d1*(valn[j]*valpm[3*j+2]+valpn[3*j+2]*valm[j]);
        }
      }
    }
    ...
    for (int n=m+1;n<natoms;n++)
    {
      ...
      copy_grid(gsa,grid2a,grid);
      recenter_grid_zero(gsa,grid2a,-A2,-B2,-C2);
      ...
      for (int i2=s3;i2<s4;i2++)
      {
        ...
        eval_sh(ii2,gsa,grid2a,val2[ii2],n2,l2,m2,zeta2);
        eval_p(gsa,grid2a,val2p[ii2],n2,l2,m2,zeta2);
      }
  ...
  if (drho!=NULL)
  #pragma acc parallel loop present(drho[0:gsa],delta[0:gsa3])  
  for (int m=0;m<gsa;m++)
    drho[m] = delta[3*m+0]*delta[3*m+0]+delta[3*m+1]*delta[3*m+1]+delta[3*m+2]*delta[3*m+2];


 //cleanup
  #pragma acc exit data delete(grid1a[0:gsa6],grid2a[0:gsa6],delta[0:gsa3])
...
void compute_fxc(int natoms, int* atno, float* coords, vector<vector<double> > &basis, bool gga, double* vxc, double* vxcs, int gsa, float* grid, float* wt, double* fxc, int prl)
{
  ...
  #pragma acc enter data create(grid1[0:gsa6],grid2[0:gsa6])
  #pragma acc enter data create(val1[0:iN][0:gsa],val2[0:iN][0:gsa])
  if (gga)
  {
   ...
   //single-atom elements over grid
    for (int i1=s1;i1<s2;i1++)
    {
      ...
      for (int i2=s1;i2<s2;i2++)
      {
        ...
        double valt = 0.;
        #pragma acc parallel loop present(vxc[0:gsa],valm[0:gsa],valn[0:gsa]) reduction(+:valt)
        for (int j=0;j<gsa;j++)
          valt += valn[j]*valm[j]*vxc[j];

        if (gga)
        #pragma acc parallel loop present(vxcs[0:gsa],valm[0:gsa],valpm[0:gsa3],valn[0:gsa],valpn[0:gsa3]) reduction(+:valt)
        for (int j=0;j<gsa;j++)
        {
          double valx = valn[j]*valpm[3*j+0]+valpn[3*j+0]*valm[j];
          double valy = valn[j]*valpm[3*j+1]+valpn[3*j+1]*valm[j];
 ...
 //cleanup
  #pragma acc exit data delete(grid1[0:gsa6],grid2[0:gsa6])
  #pragma acc exit data delete(val1[0:iN][0:gsa],val2[0:iN][0:gsa])
  if (gga)
  {
    #pragma acc exit data delete(val1p[0:iN][0:gsa3],val2p[0:iN][0:gsa])
  }
